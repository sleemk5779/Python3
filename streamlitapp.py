...
def app():
    ...
    if uploaded_file is not None:
        ...
        def exit_app():
            st.write("")  # Spacer
            if st.button("Exit App", type="primary"):
                st.markdown(
                    """
                    <script>
                        window.close();
            ...
            try:
                ...
                if run_btn:
                    with st.spinner("Generating report..."):
                        ...
                        df['Calculated FTE'] = ((df['Contact Hours'] * 16 * df['FTE Count']) / 512).round(3)
                        df['Generated FTE'] = ((df['Tier Value'] + 1926) * df['Calculated FTE']).round(2)
                        df['Enrollment Per'] = df.apply(
                            lambda row: (row['FTE Count'] / row['Capacity']) if row['Capacity'] != 0 else 0, axis=1
                        ).round(4)

                        course_df = df[df['Course Code'].str.upper() == course_code.upper()]
                        top_fte = course_df.nlargest(10, 'Generated FTE')
